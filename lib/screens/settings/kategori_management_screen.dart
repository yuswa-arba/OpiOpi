...
class KategoriManagementScreen extends StatefulWidget {
  const KategoriManagementScreen({super.key});

  @override
  State<KategoriManagementScreen> createState() =>
      _KategoriManagementScreenState();
}

class _KategoriManagementScreenState extends State<KategoriManagementScreen> {
  ...
  Future<void> _add() async {
    final name = await _showDialog(title: 'Tambah Kategori');
  ...
  Future<void> _edit(int rowIndex, String current) async {
    final name = await _showDialog(title: 'Edit Kategori', initial: current);
...
class _CrudItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.label_rounded, size: 20, color: AppTheme.primary),
